 ...
 * @author Onur Karaduman (onrkrdmn@gmail.com), Frederik beuth(beuth@hrz.tu-chemnitz.de)
 * 
 * @details
...
public class InterpolateAnimations
{
  	...
  	public bool animate(float targetX, float targetY, float targetZ)
	{
		 ...
		 if(CalculateAnimateWeights(targetX, targetY, targetZ))
		{
	         ...
	         Agent.animation.Blend(animationArray[minTriangle.vertex1], s);
	         Agent.animation.Blend(animationArray[minTriangle.vertex2], t);
	         Agent.animation.Blend(animationArray[minTriangle.vertex3], u);
			 return true;
		}
		else
		{
